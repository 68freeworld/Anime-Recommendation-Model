"""Fill missing poster URLs using the AniList GraphQL API."""
...
def main() -> None:
    """Load anime_df_final.pk and populate a cover_url column."""
    df = pd.read_pickle("anime_df_final.pk")

    if "cover_url" not in df.columns:
        df["cover_url"] = ""

    mask = df["cover_url"].isna() | df["cover_url"].astype(str).eq("")
    missing_ids = df.loc[mask, "anime_id"].astype(int).tolist()
    total_rows = len(df)
    ...
    for batch in chunks(missing_ids, 50):
        ...
        for media in data.get("data", {}).get("Page", {}).get("media", []):
            ...
            df.loc[df["anime_id"] == mal_id, "cover_url"] = url
    ...
    print(
        f"Updated {updated} / {total_rows} rows \u2013 wrote anime_with_covers.csv"
    )
